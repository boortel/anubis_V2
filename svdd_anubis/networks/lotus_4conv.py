...
class LOTUS_4conv(BaseNet):
    ...
    def __init__(self, width, height, rep_dim = 128):
        ...
        self.fc1 = nn.Linear(256 * int(self.width/16) * int(self.height/16), self.rep_dim, bias=False) # 20 * 15
...
class LOTUS_Autoencoder_4conv(BaseNet):
    ...
    def __init__(self, width, height, rep_dim = 128):
        ...
        nn.init.xavier_uniform_(self.conv3.weight, gain=nn.init.calculate_gain('leaky_relu'))
        self.bn2d4 = nn.BatchNorm2d(256, eps=1e-04, affine=False)
        self.fc1 = nn.Linear(256 * int(self.width/16) * int(self.height/16), self.rep_dim, bias=False)
        self.bn1d = nn.BatchNorm1d(self.rep_dim, eps=1e-04, affine=False)

        # Decoder
        self.deconv1 = nn.ConvTranspose2d(int(self.rep_dim / (self.width/16 * self.height/16)), 256, 5, bias=False, padding=2)
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), int(self.rep_dim / (self.width/16 * self.height/16)), int(self.width/16), int(self.height/16))
        x = F.leaky_relu(x)
        x = self.deconv1(x)
